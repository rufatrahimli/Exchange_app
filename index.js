...
document.querySelector(".arrows").onclick = () => {
  let activeLeft = document
    .querySelector(".currency .active")
    .getAttribute("data-id");
  let activeRight = document
    .querySelector(".currency_1 .active")
    .getAttribute("data-id");
  ...
  if (activeRight == 5) {
    document.querySelector(`.currency [data-id="${activeRight}"]`).value =
      selectRightValue;
  }
  if (activeLeft == 5) {
    document.querySelector(`.currency_1 [data-id="${activeLeft}"]`).value =
      selectLeftValue;
  }

  document.querySelector(".currency .active").classList.remove("active");
  document
    .querySelector(`.currency [data-id="${activeRight}"]`)
    .classList.add("active");

  document.querySelector(".currency_1 .active").classList.remove("active");
  document
    .querySelector(`.currency_1 [data-id="${activeLeft}"]`)
    .classList.add("active");
  currencyChange();
};

document
  .querySelectorAll(".currency_value")
  .forEach((item) => (item.onclick = currencyChange));
document
  .querySelectorAll(".currency-value")
  .forEach((item) => (item.onchange = currencyChange));
...
async function currencyChange() {
  ...
  try {
    ...
    document.querySelector(".rate1").textContent = `1 ${to} = ${obj.info.rate}`;
  } catch (error) {
    console.error(error);
  }
}
